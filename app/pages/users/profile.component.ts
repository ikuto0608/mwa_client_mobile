import { Component, OnInit, AfterViewInit, ViewChild, ElementRef, trigger, style, animate, state, transition, keyframes } from '@angular/core';
...
export class UsersProfileComponent implements OnInit, AfterViewInit {
  public user: User;
  public exams: Array<Exam>;
  public latestRecords: any;
  public examName: any;
  public averageScore: any;
  public averageRecordTime: any;
  ...
  getContents(data: any) {
    this.user = User.toUser(data.user);
    this.exams = data.exams.map(examJson => Exam.toExam(examJson));
  }

  showHistoryDetail(exam: Exam) {
    this.examName = exam.name;

    this.userService
        .latestHistory(exam.id)
        .subscribe(
          (data) => this.getHistory(data),
          (err) => console.log(err),
          () => this.afterShowHistoryDetail()
        )
  }

  afterShowHistoryDetail() {
    this.isOn = !this.isOn;
    this.isDetailView = true;
  }

  showHistoryList(args: SwipeGestureEventData) {
  ...
  getHistory(data: any) {
    ...
    this.averageRecordTime = data.averageRecordTime;
  }
